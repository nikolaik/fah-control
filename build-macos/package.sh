#!/bin/bash
#
# Package script for fah-control.
#
# Thank you gaphor <3

set -euo pipefail

# Also fix $INSTALLDIR/MacOS/FAHControl in case this number changes
APP=FAHControl.app
...
rm -rf FAHControl.app FAHControl-*.dmg FAHControl-*-macos.zip


# Make a virtual env, so we are not bothered with site-packages installed on the host system

python3 -m venv --copies --prompt FAHControl.app "${APPHOME}"
source "${APPHOME}/bin/activate"

VERSION="$(cat ../fah/Version.py | cut -d' ' -f3 | tr -d "'")"
...
cp ../images/FAHControl.icns "${RESOURCESDIR}"
# cat Info.plist | sed 's#VERSION#'${VERSION}'#g' > "${INSTALLDIR}/Info.plist"
cat ../FAHControl > "${MACOSDIR}/FAHControl"
chmod +x "${MACOSDIR}/FAHControl"
...
log "Installing FAHControl in ${INSTALLDIR}..."

pip3 install --no-warn-script-location ../dist/FAHControl-${VERSION}-py3-none-any.whl
...
function fix_paths {
  ...
  do
    ...
    # @executable_path is /path/to/FAHControl.app/MacOS
...
log "Building FAHControl-$VERSION.dmg..."

hdiutil create -srcfolder $APP FAHControl-$VERSION.dmg
